...
interface EditPromotionDialogProps {
  ...
  onUpdate: (id: number, promotion: Omit<Promotion, 'id' | 'soLanDaSuDung' | 'usedCount'>) => void;
}
...
export default function EditPromotionDialog({
  ...
  useEffect(() => {
    if (promotion) {
      // Parse ISO 8601 DateTime to date format for input
      const parseDate = (dateStr: string) => {
        if (!dateStr) return '';
        // Handle ISO 8601 format: "2025-12-01T00:00:00" or "2025-12-01"
        const datePart = dateStr.split('T')[0];
        return datePart;
      };

      setFormData({
        maKhuyenMai: promotion.maKhuyenMai || promotion.code || '',
        tenKhuyenMai: promotion.tenKhuyenMai || promotion.name || '',
        moTa: promotion.moTa || promotion.description || '',
        loaiKhuyenMai: promotion.loaiKhuyenMai || promotion.type || 'PERCENTAGE',
        giaTriKhuyenMai: promotion.giaTriKhuyenMai || promotion.value || 0,
        giaTriToiThieu: promotion.giaTriToiThieu || promotion.minPurchaseAmount || 0,
        giamToiDa: promotion.giamToiDa || promotion.maxDiscountAmount || 0,
        ngayBatDau: parseDate(promotion.ngayBatDau || promotion.startDate || '') || new Date().toISOString().split('T')[0],
        ngayKetThuc: parseDate(promotion.ngayKetThuc || promotion.endDate || '') || '',
        chiNhanhId: promotion.chiNhanhId || promotion.branchId || 1,
        tenChiNhanh: promotion.tenChiNhanh || promotion.branchName || 'Chi nhánh Trung tâm',
        trangThai: promotion.trangThai || (promotion.isActive ? 'ACTIVE' : 'INACTIVE'),
        soLanSuDungToiDa: promotion.soLanSuDungToiDa || promotion.usageLimit,
      });
    }
  }, [promotion]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!promotion || !formData.maKhuyenMai || !formData.tenKhuyenMai || !formData.giaTriKhuyenMai || !formData.ngayKetThuc) {
      return;
    }
    ...
    onUpdate(promotion.id, promotionData);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-gray-900">Chỉnh sửa khuyến mãi</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="edit-maKhuyenMai" className="mb-2 block">
            ...
            <div>
              <Label htmlFor="edit-loaiKhuyenMai" className="mb-2 block">
              ...
              >
                <SelectTrigger id="edit-loaiKhuyenMai">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div>
            <Label htmlFor="edit-tenKhuyenMai" className="mb-2 block">
              Tên khuyến mãi <span className="text-red-500">*</span>
            </Label>
            <Input
              id="edit-tenKhuyenMai"
          ...
          <div>
            <Label htmlFor="edit-moTa" className="mb-2 block">
              Mô tả
            </Label>
            <Input
              id="edit-moTa"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="edit-giaTriKhuyenMai" className="mb-2 block">
                Giá trị giảm <span className="text-red-500">*</span>
              </Label>
              <Input
                id="edit-giaTriKhuyenMai"
                type="number"
            ...
            <div>
              <Label htmlFor="edit-giaTriToiThieu" className="mb-2 block">
                Đơn hàng tối thiểu (₫)
              </Label>
              <Input
                id="edit-giaTriToiThieu"
                type="number"
          ...
          {formData.loaiKhuyenMai === 'PERCENTAGE' && (
            <div>
              <Label htmlFor="edit-giamToiDa" className="mb-2 block">
                Giảm tối đa (₫)
              </Label>
              <Input
                id="edit-giamToiDa"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="edit-ngayKetThuc" className="mb-2 block">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="edit-soLanSuDungToiDa" className="mb-2 block">
                Giới hạn sử dụng
              </Label>
              <Input
                id="edit-soLanSuDungToiDa"
                type="number"
